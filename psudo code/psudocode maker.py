...
def wordReplacer(
    svgfile, linesToAvoid, clues, charCheck=True, removeEndChar=False
):  # Charcheck can be used to disable specific character checking, will remove words in middle of things though!
    """
    ...
    for count in range(0, len(svgfile)):
        for i in range(0, len(clues)):
            found = svgfile[count].find(
                clues[i][0]
            )  # Check if the current line in the SVG file has the required string
            if (not (found == -1)) and not (count in linesToAvoid):
                if (
                    (found > 0)
                    and (len(svgfile[count]) > (found + len(clues[i][0])))
                    and charCheck
                ):
                    ...
                    if count == 14:
                        pass
                    if (
                        (svgfile[count][found - 1] == "(")
                        or (svgfile[count][found - 1] == " ")
                        or (svgfile[count][found - 1] == '"')
                        or (svgfile[count][found - 1] == ".")
                    ) and (
                        (svgfile[count][found + len(clues[i][0])] == "(")
                        or (svgfile[count][found + len(clues[i][0])] == " ")
                        or (svgfile[count][found + len(clues[i][0])] == '"')
                        or (svgfile[count][found + len(clues[i][0])] == ".")
                    ):
                        firstbit = svgfile[count][:found]
                        secondbit = svgfile[count][
                            found + len(clues[i][0]) : len(svgfile[count])
                        ]
...
def writeTextFile(
    svgfile, name="/Users/andrew/PycharmProjects/Experiments/psudocode/pythonfixed.py"
):
    """
...
def indentationFinder(svgfile, linesToAvoid):
    ...
    for count in range(len(svgfile)):  # Iterate through each line in the text file
        for i in range(
            len(searchFor)
        ):  # For each line in text file, iterate through clues
            ...
            if (not (found == -1)) and not (count in linesToAvoid):
                ...
                # time.sleep(0.1)
                distance = (
                    found  # Distance is basically how many characters it is indented in
                )
                lineDone = False
                for a in range(
                    count + 1, len(svgfile)
    ...
    for i in range(0, len(svgfile)):  # iterate through the text file
        for count in range(
            ...
            if i in searchFor[count][2]:  # Checks if this line is being requested
                for x in range(
                    0, len(searchFor[count][2])
                ):  # If it is, lets iterate through and find the exact reference
                    if (
                        searchFor[count][2][x] == i
                    ):  # Checks if it is the exact reference
                        workingWith = x
                        indented = ""
...
def main(filename):
    ...
    svgfile = blankLineRemover(
        svgfile
    )  # Removes all blank lines from the file, makes easier to work with
    linesToAvoid = multiLineCommentTracker(
        svgfile
    )  # Adds lines to be ignored by the replacers, mainly just multiline comments
    ...
    for i in range(0, 6):
        svgfile = wordReplacer(
            svgfile, linesToAvoid, clues
        )  # Replaces every use of elif with ~~~~ to avoid confusing the indentation finder laster

    searchfor = indentationFinder(
        svgfile, linesToAvoid
    )  # Traces indentation through the file. It finds the start and end of structures, e.g. functions, loops etc. Returns a list containing locations of all of the items being searched for in the file.

    svgfile = rebuildList(
        svgfile, searchfor, removeLines(svgfile, ["debug", "info", "warning", "#print"])
    )  # Rebuilds the new text file based off the changes from indentationFinder
    ...
    svgfile = wordReplacer(
        svgfile, linesToAvoid, [["OUTPUT(", "OUTPUT ", ")"]], False, True
    )
