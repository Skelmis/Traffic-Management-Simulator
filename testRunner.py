        ...
        for i, x in enumerate(row):
            for j, y in enumerate(x.split("\n")):
                if len(y) == 0 and self.allow_newlines == False:
                    continue
                try:
                    self.__columnSize__[i] = max(
                        self.__columnSize__[i], self.__len__(y)
                    )
                except IndexError:
    ...
    def __repr__(self):
        ...
        for x in self.__rows__:
            ...
            for i, c in enumerate(x):
                x[i] = (
                    c.ljust(self.__columnSize__[i])
                    + (len(c) - self.__len__(c) - 3) * " "
                )
...
class TestRunner(Template_mixin):
    ...
    def _generate_report(self, result):
        ...
        tests = ""
        for cid, (testClass, classResults) in enumerate(
            sortedResult
            ...
            for tid, (n, test, output, error) in enumerate(
                classResults
            ):  # Iterate over the unit tests
                classTable.addRow(
                    self._generate_report_test(cid, tid, n, test, output, error)
                )
    ...
    def _generate_report_test(self, cid, tid, n, test, output, error):
        ...
        script = self.REPORT_TEST_OUTPUT_TMPL % dict(output=uo + ue,)
